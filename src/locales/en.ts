const en = {
  translation: {
    nav: {
      home: "Home",
      about: "About Us",
      services: "Services",
      projects: "Projects",
      pricing: "Pricing",
      contact: "Contact",
      getQuote: "Get Quote",
    },
    common: {
      languages: "Languages",
      viewOriginal: "View original",
      poster: "Package poster",
      notFoundPosterTitle: "Poster image not found",
      notFoundPosterBody:
        "Please save the poster as {{file}} and place it in the project {{dir}} folder, then redeploy to display it.",
      importantNotes: "Important notes",
    },
    home: {
      sectionLabels: {
        residentialCommercial: "Residential & Commercial",
        localExpertise: "Local Expertise",
      },
      hero: {
        bgAlt: "Thailand residential solar for villas",
        badge: "Local service · Japanese quality · China-backed value",
        titleLine1: "Your home energy",
        titleHighlight: "Made cleaner in Thailand",
        subtitle:
          "Over 90% of our customers are villa and home owners. We keep the process clear and practical, from savings estimates and PEA grid connection to installation and local after-sales support.",
        ctaPrimary: "Get a free home solar plan",
        ctaSecondary: "See nearby projects",
      },
      stats: {
        items: [
          { label: "Homes & villas served", value: "1,000+", unit: "sites" },
          { label: "Typical system size", value: "5–10", unit: "kW" },
          { label: "Resorts / small business", value: "50+", unit: "sites" },
          { label: "Local service hubs", value: "4", unit: "cities" },
        ],
      },
      solutions: {
        title: "Tailored for your <accent>home & business</accent>",
        promise: "Our promise: >40% savings, payback under 4.5 years",
        cards: [
          {
            title: "Residential Solar",
            desc: "3kW–10kW villa rooftop systems sized around daily loads and AC usage, with clean installation that protects the existing roof.",
          },
          {
            title: "Small Commercial",
            desc: "10kW–100kW systems for resorts, hotels, and apartments, designed to reduce high daytime electricity costs.",
          },
          {
            title: "PEA Grid Connection",
            desc: "We know PEA policies in Chiang Mai, Pattaya, and more. End-to-end paperwork so you go solar legally and smoothly.",
          },
          {
            title: "Local O&M",
            desc: "On-the-ground teams in the North and coastal areas. Regular cleaning and inverter maintenance, with fast local response.",
          },
        ],
      },
      pricingPreview: {
        title: "Residential packages & pricing",
        desc: "To help you quickly understand budget and payback, here are the most common 3kW / 6kW / 9kW reference packages. Final pricing is based on an on-site survey and written quote.",
        packages: [
          { kw: "3 kW", title: "Small home", price: "฿ 89,000", gen: "~360 kWh/mo" },
          { kw: "6 kW", title: "Family home", price: "฿ 139,000", gen: "~720 kWh/mo" },
          { kw: "9 kW", title: "Premium villa", price: "฿ 198,000", gen: "~1,080 kWh/mo" },
        ],
        highlights: ["Includes PEA grid connection", "Local warranty & support"],
        viewDetails: "View package details",
        getAccurateQuote: "Get an accurate quote",
      },
      why: {
        titleTop: "Why choose",
        titleBottom: "ColaSola in Thailand?",
        intro:
          "ColaSola brings 10 years of solar experience to Thailand, combining Japanese installation discipline, China-backed supply-chain value, and fast local service.",
        bullets: [
          "Focused: 90% of projects are villas & homes — we understand your roof",
          "Responsive: local teams for timely site visits and after-sales support",
          "PEA experts: grid-connection process & policy handled end-to-end",
          "Transparent pricing: no hidden fees, 10-year warranty, replace-not-repair policy for quality issues",
        ],
        learnTeam: "Meet our local team",
        imageAlt: "Local solar installation in Thailand",
        cardTitle: "Top choice in Chiang Mai / Pattaya",
        cardDesc: "Over 1,000 households outside metro areas have achieved energy freedom with us.",
      },
      cta: {
        title: "Have a villa in Pattaya or Chiang Mai?",
        desc: "Contact our local experts for a free site survey and a tailored solar system design.",
        button: "Get a quote now",
      },
    },
    about: {
      headerTitle: "About ColaSola",
      headerDesc:
        "With 10 years of solar experience, we combine Japanese installation discipline, China-backed supply-chain value, and responsive local service in Thailand.",
      storyTitle: "Our story",
      storyP1:
        "ColaSola started with a simple idea: going solar shouldn’t be a complex, expensive transaction — it should be as easy and accessible as buying a cola.",
      storyP2:
        "After years in the Thailand market, we saw many companies overwhelm customers with specs and vague savings promises that didn’t match reality.",
      storyP3:
        "So we chose a different path: <strong>we take responsibility for results.</strong> After an energy audit, we explain the expected savings, payback period, and warranty clearly before installation begins.",
      missionTitle: "Our mission",
      missionDesc:
        "Deliver the most efficient and practical residential solar solutions for Thai households, lowering electricity costs and promoting a greener lifestyle.",
      visionTitle: "Our vision",
      visionDesc:
        "Become the most trusted localized clean-energy partner in Chiang Mai and Pattaya — making clean energy as accessible as a cola: reasonably priced, always available, and fast to serve.",
    },
    services: {
      headerTitle: "Services",
      headerDesc:
        "ColaSola focuses on villas and small commercial solar. From on-site survey and PEA grid connection to installation, we provide worry-free, one-stop localized service.",
      items: [
        {
          title: "Site survey & custom design",
          desc: "Tailored for villa rooftops in Thailand",
          features: [
            "Free on-site visits by local engineers (Chiang Mai / Pattaya and more)",
            "Shading and roof-load analysis",
            "Generation and payback simulation",
            "Aesthetic design that preserves your villa’s look",
          ],
        },
        {
          title: "PEA grid-connection handling",
          desc: "Experienced with PEA requirements",
          features: ["Handle PEA approvals end-to-end", "Single/three-phase meter upgrade support", "Loan policy guidance", "Tax refund / incentive application support"],
        },
        {
          title: "Professional construction & installation",
          desc: "Standardized residential delivery",
          features: ["In-house teams in Chiang Mai / Pattaya (no subcontracting)", "Tier-1 modules & microinverters", "Leak-proofing and wind reinforcement", "Typically completed in 2–5 business days"],
        },
        {
          title: "Local after-sales & O&M",
          desc: "Nearby, on-call support",
          features: ["Mobile app for real-time monitoring", "Fast on-site support by local teams", "Regular panel cleaning packages", "Up to 10-year equipment warranty"],
        },
      ],
      faqTitle: "FAQ",
      faqSubtitle: "Answers to the most common questions about home solar in Thailand",
      faqs: [
        {
          q: "How much does a home solar system cost in Chiang Mai / Phuket?",
          a: "For mid-to-large villas, a 5kW–10kW system typically costs around ฿120,000–฿220,000. Final pricing depends on roof structure, grid-connection conditions, and equipment brands. We offer free on-site surveys and quotes.",
        },
        {
          q: "What’s the typical payback period (ROI)?",
          a: "With current Thai electricity rates, residential solar payback is usually 4–5 years. If you run AC more during daytime, payback can be faster.",
        },
        {
          q: "Do you handle the PEA grid-connection application?",
          a: "Yes. We specialize in PEA grid-connection in provinces such as Chiang Mai, Chiang Rai, Phuket, and Pattaya. From drawings and meter changes to final approval, we handle everything for you.",
        },
        {
          q: "Can you help with financing?",
          a: "Yes. We can help customers review and apply for suitable financing options, including zero-down and low-interest plans, with support throughout the process.",
        },
      ],
      ctaTitle: "Want to see what we can do for you?",
      ctaDesc: "Contact our local team to get a free site survey and a tailored quote.",
      ctaButton: "Get a free plan",
    },
    projects: {
      headerTitle: "Projects",
      headerDesc:
        "From sea-view villas in Pattaya to mountain homes in Chiang Mai — we’ve delivered reliable green energy for thousands of households and boutique businesses across Thailand.",
      types: {
        residential: "Residential rooftop",
        commercial: "Small commercial",
      },
      items: [
        { title: "Phuket sea-view villa solar", type: "residential" },
        { title: "Doi Suthep hillside villa, Chiang Mai", type: "residential" },
        { title: "Boutique resort hotel, Pattaya", type: "commercial" },
        { title: "Organic farm homestay, Chiang Rai", type: "commercial" },
        { title: "Modern private home, Rawai Phuket", type: "residential" },
        { title: "Custom-built family home, Hang Dong", type: "residential" },
      ],
    },
    pricing: {
      headerTitle: "Residential packages & pricing",
      headerDesc:
        "Below are ColaSola’s common 3kW / 6kW / 9kW residential reference packages. Final pricing depends on roof structure, usage habits, cable routing distance, and grid-connection conditions.",
      getAccurateQuote: "Get an accurate quote",
      downloadPoster: "Download poster",
      packages: [
        {
          title: "Small home",
          kw: "3 kW",
          price: "฿ 89,000",
          generation: "~360 kWh/mo",
          highlights: ["0% down (subject to final plan)", "Grid-tied", "PEA grid-connection included", "Local warranty & support"],
        },
        {
          title: "Family home",
          kw: "6 kW",
          price: "฿ 139,000",
          generation: "~720 kWh/mo",
          highlights: ["0% down (subject to final plan)", "Grid-tied", "PEA grid-connection included", "Local warranty & support"],
        },
        {
          title: "Premium villa",
          kw: "9 kW",
          price: "฿ 198,000",
          generation: "~1,080 kWh/mo",
          highlights: ["0% down (subject to final plan)", "Grid-tied", "PEA grid-connection included", "Local warranty & support"],
        },
      ],
      notes: [
        "Prices are for reference; the final price is based on an on-site survey and written quote.",
        "Monthly generation is an estimate and depends on orientation, shading, and local irradiance.",
        "If you’re in Chiang Mai / Chiang Rai / Phuket / Pattaya, we can arrange a local on-site evaluation.",
      ],
    },
    contact: {
      headerTitle: "Contact",
      headerDesc:
        "We’re ready to provide professional consultation and technical support. Fill out the form or call us — we’ll get back to you as soon as possible.",
      infoTitle: "Local service team",
      infoDesc:
        "Our experts are based in Chiang Mai and Pattaya, enabling fast response across Northern Thailand and coastal resort areas.",
      officeHq: "Chiang Mai HQ",
      officeBranch: "Pattaya branch",
      phoneTitle: "Phone",
      phoneNote: "(Chinese / English / Thai / Japanese supported)",
      emailTitle: "Email",
      hoursTitle: "Business hours",
      hoursLine1: "Mon–Fri: 9:00 AM – 6:00 PM",
      hoursLine2: "Closed on weekends & holidays",
      formTitle: "Message us",
      successTitle: "Submitted!",
      successDesc: "Thanks for your message. Our consultant will contact you within 24 hours.",
      fields: {
        name: "Name",
        phone: "Phone",
        email: "Email",
        company: "Company",
        message: "Project details",
      },
      validation: {
        nameRequired: "Please enter your name",
        phoneRequired: "Please enter your phone number",
        messageRequired: "Please describe your needs",
        emailInvalid: "Invalid email format",
      },
      placeholders: {
        message:
          "Briefly describe your project, e.g., roof area, monthly usage, desired system size, etc...",
      },
      sending: "Submitting...",
      send: "Send message",
    },
    footer: {
      brandDesc:
        "We are a leading solar integrator in Thailand with service hubs in Chiang Mai, Phuket, and Pattaya, delivering worry-free green energy solutions for villas, homes, and boutique businesses.",
      quickLinks: "Quick links",
      coreServices: "Services",
      contact: "Contact",
      serviceList: ["Engineering", "Procurement & Supply Chain", "Construction Management", "O&M Services", "Energy storage solutions"],
      chiangMaiHq: "Chiang Mai HQ:",
      pattayaBranch: "Pattaya branch:",
      privacy: "Privacy policy",
      terms: "Terms of service",
    },
  },
} as const;

export default en;
